...
void Friend::followPlayerX(Player player) {
	//Abstand zum Spieler
	int posToPlayer;
	if (player.facing == player.fLeft) {
		posToPlayer = 50;
	}
	else {
		posToPlayer = -(30+width);
	}
	//
	if (player.getHitbox().x < posX - posToPlayer) {
		moveLeft(speed, false);
	}
	if (player.getHitbox().x > posX - posToPlayer) {
		moveRight(speed, false);
...
void Friend::followPlayerY(Player player) {
	if (player.getHitbox().y + player.getHeight() / 2 < posY + 30){
		moveUp(2, false);
	}
	if (player.getHitbox().y + player.getHeight() / 6 > posY + 30) {
		if (player.state == player.sGliding) {
			moveDown(1, false);
		}
		else {
			moveDown(3, false);
