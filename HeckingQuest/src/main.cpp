...
int main(int argc, char* args[]) {	
	...
	Wall.push_back({ 1950, LEVEL_HEIGHT - 400, 70, 80 });
	Wall.push_back({ 1700, LEVEL_HEIGHT - 520, 110, 30 });
	Wall.push_back({ 1300, LEVEL_HEIGHT - 600, 100, 30 });
	Wall.push_back({ LEVEL_WIDTH-80, LEVEL_HEIGHT - 550, 80, LEVEL_HEIGHT-(LEVEL_HEIGHT - 550)});
	...
	while (!quit) {
		...
		//Friend
		f1.currentFrame = f1.floatFrameclip[f1.floatFrame / f1.FLOAT_FRAMES_SPEED];
		if (f1.floatFrame >= f1.FLOAT_FRAMES_COUNT*f1.FLOAT_FRAMES_SPEED - 1) {
			f1.floatFrame = 0;
		}
		...
		f1.floatFrame++;
		if (p1.getPosY() - f1.getPosY() > 30 || f1.getPosY() - p1.getPosY() > 30) {
			f1.floatFlag = 0;
			f1.followPlayerY(p1);		
		}
		//if (p1.getPosX() - f1.getPosX() > 50 || f1.getPosX() - p1.getPosX() > 40 + f1.getWidth()) {
			f1.followPlayerX(p1);
		//}
		/* === === === === === === === === === */		
		...
		f1.render(f1.getPosX(), f1.getPosY(), f1.getWidth(), f1.getHeight(), &f1.currentFrame, f1.flipType);
...
bool loadMedia() {
	...
	if (!friendTex.loadTexture("sprites/eyyy.png")) {
		std::cerr << "FriendWimp Textur konnte nicht geladen werden! Error: " << SDL_GetError() << std::endl;
	}
...
void wimp_init() {
	...
	f1.setTexture(friendTex.getTexture());
	f1.setHeight(30);
	f1.setWidth(30);
}
...
void wimp_setSprites() {
	...
	//
	f1.floatFrameclip[0].x = 0;
	f1.floatFrameclip[0].y = 0;
	f1.floatFrameclip[0].w = 45;
	f1.floatFrameclip[0].h = 45;

	f1.floatFrameclip[1].x = 45;
	f1.floatFrameclip[1].y = 0;
	f1.floatFrameclip[1].w = 45;
	f1.floatFrameclip[1].h = 45;
}
